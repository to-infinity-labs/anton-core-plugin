...
Single ADR-0010 envelope. The default form is `{"status":"ok","report":{"severity":...,"checks":[...],"summary":...}}` — only `severity`, `checks`, and `summary` are always present. `--full` extends `report` with the `counts`, `memory`, `remediation`, and `system` panels. Severity rolls up per-panel statuses: any `critical` → `critical`; any `warning` or `degraded` → `warning`; otherwise `healthy`. Contract: [docs/plugin-spec/05-cli-contract.md#report-health](../../docs/plugin-spec/05-cli-contract.md#report-health).
